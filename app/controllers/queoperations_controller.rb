#### author :Upender
### This Controller is used to display the questions in iframe.
class QueoperationsController < ApplicationController
  layout nil
 ...
 @qId = params[:qId]
  @secTemplateId=params[:secTId].to_s
    ######### identifying the record to update current question needs , session variables curent_user_id,examId,examTemplateId,sectionTemplateId
    @studentPresentSection  =  Viewactivesection.find(session[:user_id],session[:examId],session[:examTemplateId],session[:examDayNumber],params[:secTId],:select=>"NumQuestionsInSection,NumQuestionsAttempted")
    ######### Updating the current Question Code on clicking the QuestionNumber
     Viewactivesection.update_column('CurrentQuestionCode', params[:qCode],session[:user_id],session[:examId],session[:examTemplateId],session[:examDayNumber],params[:secTId])
    @numQuestionsAttempted =@studentPresentSection.NumQuestionsAttempted
  end
  def get_previous_question_by_srNo
    @studentPresentSection  =  Viewactivesection.find(session[:user_id],session[:examId],session[:examTemplateId],session[:examDayNumber],params[:secTId],:select=>"CurrentQuestionPointer,SectionPaper")
